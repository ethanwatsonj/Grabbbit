        ...
        VStack(alignment: .leading, spacing: DesignTokens.Spacing.md) {
            ...
            VStack(spacing: 0) {
                ...
                DiagramNode(
                    ...
                    detail: "isCloudConnected is true when Keychain returns a non-empty key. UI shows Connected / Disconnect plus an always-visible Apple FM ↔ Gemini provider control (Gemini stays locked until a key is saved). Disconnect deletes the Keychain item; provider preference stays in UserDefaults.",
                    tone: .signal
                )
